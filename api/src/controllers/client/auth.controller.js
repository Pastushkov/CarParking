...
const register = async (req, res) => {
  const { phone, pin, userName, cars } = req.body;
  ...
  const carsWithClientId = cars.map((car) => ({
    ...car,
    clientId: client.id,
  }));

  const savedCars = await Car.insertMany(carsWithClientId);

  client.cars = savedCars.map((car) => car._id);
  await client.save();
